...
test('it shows the title in the banner', () => {
	setup();
	// We expect that the title 'Hi, I am Eyob' is in the banner component
	expect(screen.getByText(/Hi, I am Eyob/i)).toBeInTheDocument();
});
...
test('can download cv when clicked on download cv button', async () => {
	...
	// expect(downloadLink.download).toEqual('Eyob_Adugna-Resume.pdf');
	// expect(downloadLink.href).toEqual('/files/Eyob_Adugna-Resume.pdf');
